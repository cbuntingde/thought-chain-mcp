...
export class DatabaseManager {
  ...
  getThoughtChain(chainId) {
    try {
      ...
      const steps = this.db.prepare(stepsQuery).all(chainId).map(step => ({
        id: step.step_number,
        thought: step.thought,
        reflection: step.reflection,
        timestamp: step.timestamp,
    ...
    try {
      ...
      const filtered = allChains.filter(chain =>
        chain.steps.some(step => 
          ...
          (step.reflection && step.reflection.toLowerCase().includes(searchLower))
        )
      );
  ...
  getStats() {
    try {
      const chainCount = this.db.prepare('SELECT COUNT(*) as count FROM thought_chains').get();
      const stepCount = this.db.prepare('SELECT COUNT(*) as count FROM thought_steps').get();
      const activeCount = this.db.prepare("SELECT COUNT(*) as count FROM thought_chains WHERE status = 'active'").get();
